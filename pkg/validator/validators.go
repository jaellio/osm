...
// TODO(jaellio): should this return an error instead?
func (cv *configValidator) activeRootCertificateRotation(newMRC *configv1alpha2.MeshRootCertificate) bool {
	...
	if inUseMRCs >= 2 {
		log.Debug().Msgf("root certificate rotation in progress. Found %d MRCs in non error, not inactive, or empty state status", inUseMRCs)
		return true
	}
...
func validateMeshRootCertificateStatusTransition(oldMRC *configv1alpha2.MeshRootCertificate, newMRC *configv1alpha2.MeshRootCertificate) error {
	...
	// TODO(jaellio): is this check necessary?
	if oldMRC.Status.State == newMRC.Status.State {
...
// TODO(jaellio): should there be an error state mapping and an inactive state mapping?
